...
The RPM spec file provided here is for Bitcoinlimitededition-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitcoinlimitededition.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoinlimitededition git repository](https://github.com/bitcoinlimitededition/bitcoinlimitededition) but are not part of
...
Currently only `Source10` is used, for the example `bitcoinlimitededition.conf` file.
...
in the bitcoinlimitededition git repository. At present only an SVG version of the Bitcoinlimitededition
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitcoinlimitededition
...
needed when 0.12.1 is released, a proper fix is already in the Bitcoinlimitededition git
master branch.
...
idea to build Bitcoinlimitededition Core against a static version of that library compiled
...
The `bitcoinlimitededition.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitcoinlimitededition-server` package. At this point, most RPM based
...
The `bitcoinlimitededition-server` package should have SELinux support. How to properly do
...
Bitcoinlimitededition-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitcoinlimitededition/). He has been
packaging Bitcoinlimitededition for Fedora at least since 2012.
...
4. I renamed the `bitcoinlimitededition` package that contains the Qt GUI to `bitcoinlimitededition-core` as
that appears to be how the general population refers to it, in contrast to
`bitcoinlimitededition-xt` or `bitcoinlimitededition-classic`. I wanted to make sure the general population
