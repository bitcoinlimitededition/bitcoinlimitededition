#!/bin/bash
# Copyright (c) 2013 The Bitcoinlimitededition Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitcoinlimitededition database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitcoinlimitededition datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitcoinlimitededition datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitcoinlimitededition 0.7 datadir."
    ;;
  3)
    echo "Detected Bitcoinlimitededition pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitcoinlimitededition 0.8 datadir."
    ;;
esac
