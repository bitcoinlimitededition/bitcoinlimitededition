...
###   bitcoinlimitededitioncore.org and bitcoinlimitededition.org and compares them.
...
WORKINGDIR="/tmp/bitcoinlimitededition_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinlimitededitioncore.org"
HOST2="https://bitcoinlimitededition.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinlimitededition-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinlimitededition-',
...
if [ $? -ne 0 ]; then
   echo "bitcoinlimitededition.org failed to provide signature file, but bitcoinlimitededitioncore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinlimitededition.org and bitcoinlimitededitioncore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoinlimitededition Core binary release signing key installed?"
   fi
