...
//! so bitcoinlimitededition should never create different databases with the same fileid, but
...
bool CDBEnv::Open(bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of bitcoinlimitededition may be using it.\n", strPath);
        return false;
    }
...
CDB::CDB(CWalletDBWrapper& dbw, const char* pszMode, bool fFlushOnCloseIn) : pdb(nullptr), activeTxn(nullptr)
{
...

    {
        ...
        if (pdb == nullptr) {
            ...
            // bitcoinlimitededition from opening the same data file through another
